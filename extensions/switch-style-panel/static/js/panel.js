...
(function ($) {
	...
	var switchPanel = function ($panel) {
		...
		function applyStyle(event) {
			var settings = $(event.target).data('settings'),
				...
				tags = {
					...
					links: ['links', 'links_hover', 'links_active']
				};
		...
		function generateBackgroundCss(selectors, options) {
			...
			$.each(selectors, function (index, selector) {
				css += selector + '{';
				if (options['background-image'] && options['background-image']['choices'][options['background-image']['value']]['css'].hasOwnProperty('background-image')) {
