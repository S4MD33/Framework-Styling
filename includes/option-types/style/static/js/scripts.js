...
(function ($) {
	...
	fwOptionTypeStyle = {
			...
			fwEvents.on('fw:options:init', function (data) {
				...
				$options.find('.fw-option-type-style-option.predefined_styles .fw-option-type-image-picker')
					.on('fw:option-type:image-picker:clicked', function (e, data) {
						...
						$options.off('fw:option-type:color-picker:changed', 'input.fw-option-type-color-picker', fwOptionTypeStylePreview.fireColorChange);
						...
						$options.on('fw:option-type:color-picker:changed', 'input.fw-option-type-color-picker', fwOptionTypeStylePreview.fireColorChange);
					});
			...
			_.each(data, function (item, index) {
				...
				if (index === 'background') {
					...
					if (typography.hasOwnProperty(index)) {
						...
						$element.find('input.fw-option-type-color-picker.initialized').iris({
							color:item,
		...
		setBackgroundOption: function ($container, data) {
			...
			//Color
			$container.find('.fw-option-type-style-option.background-color .primary-color input.fw-option-type-color-picker.initialized').iris({
				color: data['background-color']['primary'],
				change: function(event, ui) { $(this).css('background-color',ui.color.toString()) }
			});
			$container.find('.fw-option-type-style-option.background-color .secondary-color input.fw-option-type-color-picker.initialized').iris({
				color: data['background-color']['secondary'],
				change: function(event, ui) { $(this).css('background-color',ui.color.toString()) }
			});

			//Image
			if (data['background-image'] && data['background-image'].hasOwnProperty('choices')) {
