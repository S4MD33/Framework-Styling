			...
			<div class="preview-block">
				<div class="<?php echo esc_attr($key); ?>">
					<?php for ( $i = 1; $i <= 6; $i ++ ) :
						...
						echo '<h' . $i . ' class="preview-h' . $i . '">' . $block['title'] . '(H' . $i . ')' . '</h' . $i . '>';
					endfor; ?>
					<?php if ( in_array( 'p', $block['elements'] ) ) : ?>
						<p class="block-paragraph">Mauris iaculis portititor posuef Praesent id metus massa, ut.</p>
					<?php endif; ?>
					<div class="bl-links">
						...
						<?php endif; ?>
						<?php if ( in_array( 'links_active', $block['elements'] ) ): ?>
							<a href="#" class="links_active">Active Link</a>
						<?php endif; ?>
					</div>
				</div>
			</div>
